...
class TestPredictEndpoint:
    ...
    def test_predict_returns_200(self, zidane_b64):
        resp = client.post("/predict", json={"image_base64": zidane_b64, "confidence": 0.3})
        assert resp.status_code == 200

    def test_predict_detects_at_least_one_object(self, zidane_b64):
        data = client.post("/predict", json={"image_base64": zidane_b64, "confidence": 0.3}).json()
        assert len(data["detections"]) >= 1

    def test_predict_response_schema(self, zidane_b64):
        data = client.post("/predict", json={"image_base64": zidane_b64, "confidence": 0.3}).json()
        for k in ["detections", "inference_ms", "model_used", "image_width", "image_height"]:
            assert k in data
...
class TestBatchEndpoint:
    ...
    def test_batch_returns_correct_count(self, two_images_b64):
        data = client.post("/predict/batch", json={"images_base64": two_images_b64, "confidence": 0.3}).json()
